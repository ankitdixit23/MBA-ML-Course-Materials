...
# Look variable interpretation using SHAP
explainer = shap.Explainer(model)
shap_values = explainer(X)

# SHAP for single instance
shap.plots.waterfall(shap_values[0], show=dpl)
if not dpl:
    plt.savefig(os.path.join(script_dir, "shap1.png"))
    plt.close()

# Beeswarm plot - all the SHAP values
shap.plots.beeswarm(shap_values, show=dpl)
if not dpl:
    plt.savefig(os.path.join(script_dir, "shap_beeswarm.png"))
    plt.close()

# Shap importance
clust = shap.utils.hclust(X, y, linkage="single")
shap.plots.bar(shap_values, clustering=clust, clustering_cutoff=1, show=dpl)
if not dpl:
    plt.savefig(os.path.join(script_dir, "shap_importance.png"))
    plt.close()

# SHAP dependence plot
shap.plots.scatter(shap_values,
                   ylabel="SHAP value\n(higher means more likely to renew)", show=dpl)
if not dpl:
    plt.savefig(os.path.join(script_dir, "shap_dependence.png"))
    plt.close()

## Partial dependence plots
for col in X.columns:
    shap.plots.partial_dependence(
        col, lambda x: model.predict(xgboost.DMatrix(x)), X, ice=False, 
        model_expected_value=True, feature_expected_value=True, show=dpl
    )
    if not dpl:
        plt.savefig(os.path.join(script_dir, f"pdp_{col.replace(' ', '_')}.png"))
        plt.close()
...
if not dpl:
    plt.savefig(os.path.join(script_dir, "shap_with_causal.png"))
    plt.close()
